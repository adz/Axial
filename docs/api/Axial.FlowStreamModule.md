...
A stream does nothing until a terminal operation such as `runFold`, `runForEachFlow`, or `runCollect` produces a Flow
and that Flow is run. Pulling provides backpressure: downstream requests each next value, so upstream cannot run ahead
