...
`DSL` is opt-in, so its one custom operator and short names do not enter scope unless explicitly opened. It is a thin
alias layer over `Process`; use the qualified names when shared code benefits from being more explicit. A computation
expression would add braces and custom sequencing rules to an already linear F# pipeline, so Axial deliberately uses
ordinary functions and one pipe operator instead.
