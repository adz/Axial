...
| --- | --- | --- |
| **Traces** (spans) | [`Axial.Telemetry`]({{< relref "/flow/telemetry/_index.md" >}}) emitting on the `Axial` `ActivitySource`; `Axial.Telemetry.JavaScript` on Fable targets | any `ActivityListener` — in practice the OpenTelemetry SDK; OpenTelemetry JS under Fable |
| **Logs** | the explicit `ILog` service, bridged to `Microsoft.Extensions.Logging` by [`Axial.Hosting`]({{< relref "/flow/hosting/_index.md" >}}) | your host's logging pipeline |
| **Metrics** | [`Axial.Telemetry`]({{< relref "/flow/telemetry/_index.md" >}}) — `FiberMetrics` on the `Axial` `Meter` | OpenTelemetry's `.AddMeter("Axial")`, `dotnet-counters`, the Aspire dashboard |
...
- [`Axial.Hosting`]({{< relref "/flow/hosting/_index.md" >}}) bridges `ILog` to
