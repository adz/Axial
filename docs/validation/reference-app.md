...
is a conference registration desk built with only the `Axial.Validation` package: no schemas, no Flow, and no
...
`Result<string, BadgeError>`. See [Checks]({{< relref "/validation/checks/" >}}).
...
[Result Builder]({{< relref "/validation/result-builder/" >}}).
...
[Refined]({{< relref "/validation/refined/" >}}).
...
[Validation]({{< relref "/validation/diagnostics/" >}}).
