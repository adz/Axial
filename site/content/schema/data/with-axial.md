...
takes a [`Data`]({{< relref "/schema/reference/data/t-data/" >}}) value, and every source that can become `Data` can therefore feed a schema.
...
The same schema parses the same logical input regardless of source:
[`Data.ofJsonDocument`]({{< relref "/schema/reference/data/m-data-ofjsondocument/" >}}) for a
request body, [`Data.ofNameValues`]({{< relref "/schema/reference/data/m-data-ofnamevalues/" >}}) for a form post,
[`Data.ofCliArgs`]({{< relref "/schema/reference/data/m-data-ofcliargs/" >}}) for a command line,
[`Data.ofConfiguration`]({{< relref "/schema/reference/data/m-data-ofconfiguration/" >}}) for settings. The schema never learns which one it was.
...
[`Schema.parseRetainingInput`]({{< relref "/schema/reference/schema/interpreters/m-schema-schema-parseretaininginput/" >}}) keeps the original `Data` alongside the parse result, so a failed form
...
[`Data.redisplayPath`]({{< relref "/schema/reference/data/m-data-redisplaypath/" >}}) (string form) or `Data.redisplayAt` (`DataPath` form) recovers the raw fragment
at any of them.
