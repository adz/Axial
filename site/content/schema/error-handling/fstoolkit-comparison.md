...
Axial divides the same ground by role instead of by wrapper type: plain [Result]({{< relref "/schema/reference/error-handling/result/" >}})
with your own error union for simple code, [Validation]({{< relref "/schema/reference/error-handling/validation/" >}}) when independent
failures must accumulate, and [Flow]({{< relref "/flow/reference/flow/" >}}) for effectful workflows.
[Check]({{< relref "/schema/reference/error-handling/check/" >}}) and [Refined]({{< relref "/schema/reference/error-handling/refined/" >}}) are reusable
...
- **Keep existing pure helpers**: If you have a library of Result transformation helpers
  from FsToolkit, keep using them! Axial's [`flow {}`]({{< relref "/flow/reference/flow/builders-flow.md" >}}) builders bind Result directly.
...
independent validation that should report multiple errors, use [`Validation`]({{< relref "/schema/reference/error-handling/validation/" >}}) and
[`validate {}`]({{< relref "/schema/reference/error-handling/validation/builders-validate.md" >}}) to maintain that explicit concern.
