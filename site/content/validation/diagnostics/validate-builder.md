...
[`Validation<'value, 'error>`]({{< relref "/validation/reference/validation/t-validation-validation.md" >}}) is structurally similar to `Result<'value, Diagnostics<'error>>`. You can convert between them easily:

- Use [`Validation.toResult`]({{< relref "/validation/reference/validation/m-validation-validation-toresult.md" >}}) to get a standard result back.
- Use [`Validation.fromResult`]({{< relref "/validation/reference/validation/m-validation-validation-fromresult.md" >}}), the canonical result-to-validation bridge, to start an accumulating block from an existing result.

Choose [`validate {}`]({{< relref "/validation/reference/validation/builders-validate.md" >}}) when independent
...
To build a structured report (e.g., for JSON APIs), use the [`validate.key`]({{< relref "/validation/reference/validation/m-validation-validation-key.md" >}}), `validate.index`, and `validate.name` helpers. These prefix any diagnostics produced inside the block.
