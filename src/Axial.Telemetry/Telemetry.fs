...
module FiberTelemetry =
    ...
    /// <summary>
    /// A fiber observer that records fiber defects on the <c>Axial</c> activity source: every fiber that
    ...
    /// <returns>A flow whose forked fibers report defects through the <c>Axial</c> activity source.</returns>
    ...
    /// </summary>
    /// <param name="flow">The source flow.</param>
    /// <returns>A flow whose forked fibers each produce a span on the <c>Axial</c> activity source.</returns>
    let observeWithSpans (flow: Flow<'env, 'error, 'value>) : Flow<'env, 'error, 'value> =
        Flow.withFiberObserver observerWithSpans flow

/// <summary>OpenTelemetry-compatible fiber runtime metrics on the <c>Axial</c> meter.</summary>
/// <remarks>
/// Register the meter with your OpenTelemetry pipeline (<c>AddMeter("Axial")</c>) and every metric
...
module FiberMetrics =
    ...
    let meter = new System.Diagnostics.Metrics.Meter("Axial")
    ...
    /// </summary>
    /// <param name="flow">The source flow.</param>
    /// <returns>A flow whose forked fibers report runtime metrics on the <c>Axial</c> meter.</returns>
    let observe (flow: Flow<'env, 'error, 'value>) : Flow<'env, 'error, 'value> =
