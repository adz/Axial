namespace Axial.Telemetry.JavaScript
...
/// The OpenTelemetry JS adapter for the shared span vocabulary in `Axial.Telemetry.Shared`.
...
module Otel =
    ...
    /// <c>Axial.Telemetry</c> on .NET targets.
    /// </summary>
    let traceWith
        (renderError: 'error -> string)
        (name: string)
            ...
            OnEnd =
                fun metadata defect ->
                    ...
                    | true, span ->
                        ...
                        Axial.Telemetry.Shared.SpanConventions.stampFiberEnd (JsTags.writer span) metadata.Status defect
                        span.``end`` ()
                    | _ -> ()
            OnUnobservedDefect =
                fun metadata defect ->
